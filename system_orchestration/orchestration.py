...
# 1. user input module이 실행되면 출력 결과를 기반으로 RAG를 동적으로 실행함


def adjust_rag_params(user_input: dict) -> dict:
    ...
    if "50" in num_q or "이상" in num_q:
        params["k"] = 5
        print("문항 수가 50이상 검색 범위 확장 (k=5)")
        
    if "70" in num_q or "이상" in num_q:
        params["k"] = 7
        print("문항 수가 70이상 검색 범위 확장 (k=7)")
...
def build_rag_query(user_input: dict) -> str:
...
rag_params = adjust_rag_params(user_input)
user_input = build_rag_query(user_input)
...
# 2.1 user input과 RAG 결과를 기반으로 어떤 도메인 모듈을 선택할지 고름
...
model_name = DOMAIN_MODEL_MAP.get(selected_domain, "gpt-5")
print(model_name)
